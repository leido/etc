...
function menu:init(args)
	...
	local placesmenu = {
		...
		separator,
		{ "Media",       env.fm .. " /media/dulei/bigroom", micon("folder-bookmarks") },
	}
	...
	self.mainmenu = redflat.menu({ theme = theme,
		items = {
			...
			separator,
			{ "Chrome", "google-chrome", micon("google-chrome") },
			{ "Nemo",    "nemo",    micon("folder") },
			separator,
			{ "Exit",     exitmenu,       micon("exit") },
			{ "Lock", "dm-tool lock", micon("system-shutdown") },
            { "Screenshot", "gnome-screenshot -a", micon("screenshot") }
		}
	})
